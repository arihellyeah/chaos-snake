...
*/ SNAKE.Snake = SNAKE.Snake || function() {
    ...
    function getNextHighestZIndex(myObj) {
        var highestIndex = 0, currentIndex = 0, ii;
        for(ii in myObj){
    ...
    */ return function(config) {
        ...
        // ----- private variables -----
        var me = this, playingBoard = config.playingBoard, myId = instanceNumber++, growthIncr = 5, lastMove = 1, preMove = -1, isFirstMove = true, isFirstGameMove = true, currentDirection = -1, columnShift = [
            0,
            1,
            0,
            -1
            ...
            1,
            0
        ], xPosShift = [], yPosShift = [], snakeSpeed = 80, isDead = false, isPaused = false;
        ...
        */ me.handleArrowKeys = function(keyNum) {
            ...
            if (currentDirection !== lastMove) preMove = directionFound;
        ...
        * @method go
        */ me.go = function() {
            var oldHead = me.snakeHead, newHead = me.snakeTail, grid = playingBoard.grid; // cache grid for quicker lookup
            if (isPaused === true) {
                setTimeout(function() {
                    me.go();
                }, snakeSpeed);
        ...
        */ me.eatFood = function() {
            ...
            var ii = blocks.length, index, prevNode = me.snakeTail;
            while(ii--){
            ...
            if (selectedOption.text.localeCompare("Rush") == 0) snakeSpeed > 30 ? snakeSpeed -= 5 : snakeSpeed = 30;
            return true;
        };
        /**
        ...
        */ me.reset = function() {
            ...
            var blocks = [], curNode = me.snakeHead.next, nextNode;
...
*/ SNAKE.Board = SNAKE.Board || function() {
    ...
    */ function getClientHeight() {
    ...
    return function(inputConfig) {
        // --- private variables ---
        var me = this, myId = instanceNumber++, config = inputConfig || {}, MAX_BOARD_COLS = 250, MAX_BOARD_ROWS = 250, blockWidth = 20, blockHeight = 20, GRID_FOOD_VALUE = -1, myFood, mySnake, boardState = 1, myKeyListener, isPaused = false, // Board components
        ...
        */ me.setupPlayingField = function() {
            ...
            for(var row = 0; row < numBoardRows; row++){
                me.grid[row] = [];
                for(var col = 0; col < numBoardCols; col++)if (col === 0 || row === 0 || col === numBoardCols - 1 || row === numBoardRows - 1) me.grid[row][col] = 1; // an edge
                else me.grid[row][col] = 0; // empty space
            }
            myFood.randomlyPlaceFood();
            myKeyListener = function(evt1) {
                if (!evt1) var evt1 = window.event;
                var keyNum1 = evt1.which ? evt1.which : evt1.keyCode;
                if (me.getBoardState() === 1) {
                    if (!(keyNum1 >= 37 && keyNum1 <= 40) && !(keyNum1 === 87 || keyNum1 === 65 || keyNum1 === 83 || keyNum1 === 68)) return;
                    ...
                    mySnake.rebirth();
                    mySnake.handleArrowKeys(keyNum1);
                    me.setBoardState(2); // start the game!
                    mySnake.go();
                }
                evt1.cancelBubble = true;
                if (evt1.stopPropagation) evt1.stopPropagation();
                if (evt1.preventDefault) evt1.preventDefault();
                return false;
            };
...
//# sourceMappingURL=index.72ac3200.js.map
